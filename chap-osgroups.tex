...
\begin{defn}
\emph{Semiordered semicategory action} (denoted $\supfun{}$) is a functor from an ordered semicategory~$\mathcal{C}$ to the category~$\mathbf{Pos}$.
\end{defn}
...
\[
\mathrm{C}(a,b)\Leftrightarrow f\circ a\sqsubseteq b\circ f,\quad \mathrm{C}''(a,b)\Leftrightarrow f\circ a\circ f^{\dagger}\sqsubseteq b,\quad \mathrm{C}'(a,b)\Leftrightarrow a\sqsubseteq f^{\dagger}\circ b\circ f.
\]
...
\begin{defn}
Morphism~$a$ is \emph{closed} iff $\forall x:\supfun{a}x\sqsubseteq x$.
\end{defn}
...
\end{enumerate}
(I abusee\fxerror{Seems wrong.} the notation $\id_p$ for both interspaces and for transformations; this won't lead to inconsistencies, because as proved above this mapping is faithful on restricted identities.)
...
Geometric spaces: Euclidean spaces, projective spaces, Hilbert spaces, vector spaces\fxerror{Not sure about vector spaces, see \href{https://chatgpt.com/share/6a664534-7484-83eb-864f-3999115d07a2}{ChatGPT            }.}, are characterized by their symmetry groups~$G$.
