...
\begin{defn}
\emph{Semicategory action} is a semifunctor from a semicategory to the category~$\mathbf{Set}$.
\end{defn}

The category~$\mathbf{Pos}$ is the category whose objects are (small) posets and whose morphisms are order homomorphisms.

\begin{defn}
\emph{Semiordered semicategory action} is a functor from an ordered semicategory~$\mathcal{C}$ to the category~$\mathbf{Pos}$.
\end{defn}
