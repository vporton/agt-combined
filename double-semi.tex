...
\author{Victor Porton, ORCID 0000-0001-7064-7975, Ashkelon, Israel}
...
\begin{defn}
The category~$\mathbf{SemiCatAct}$ of \emph{semicategory actions} is small semicategories~$\mathcal{C}$ together with a semifunctor $\supfun{}:\mathcal{C}\to\mathbf{Set}$ with semifunctors $F$ together with a function $G$ conforming to $\supfun{Fa}x=G\supfun{a}x$ as morphisms.
\end{defn}
...
\begin{defn}
The category~$\mathbf{HalfOrdSemiCatAct}$ of \emph{half-ordered semicategory actions} is:
\begin{description}
...
In other words, $\mathbf{OrdSemiCatAct}$ is small ordered semicategories together with a function~$\supfun{}$ to $\mathbf{Pos}$ with morphisms being a monotone semifunctors~$F$ together with a function~$G$ conforming to the identities:
\begin{enumerate}
...
I call objects of this category \emph{half-ordered semicategory actions}.
\end{defn}

\begin{defn}
\[ \mathbf{OrdSemiCatAct}\text{ is the wide subcategory of }\mathbf{HalfOrdSemiCatAct} \]
...
\begin{defn}
An \emph{actionable semicategory} is a semicategory~$\mathcal{C}$, and $\cdot$ is a binary operation on morphisms of~$\mathcal{C}$ such that $(g\circ f)\cdot x = g\cdot(f\cdot x)$.
\end{defn}
...
\begin{defn}
The category $\mathbf{Act^{\ast}SemiCat}$ of actionable semicategories are semicategory morphisms~$F$ together with a function~$G$ conforming to the identity $\supfun{Ff}x=G\supfun{f}x$.
\end{defn}

\begin{defn}
The morphisms of the category $\mathbf{OrdAct^{\ast}SemiCat}$ of ordered actionable semicategories are semifunctors that are morphisms of both $\mathbf{Act^{\ast}SemiCat}$ and $\mathbf{Pos}$ under the conditions that:
\[
...
\begin{defn}
The morphisms of the category $\mathbf{Act^{\ast}SemiCatIden}$ of actionable semicategories with identities are semifunctors that are morphism~$(F,G)$ of $\mathbf{OrdAct^{\ast}SemiCat}$ under the conditions that: $F\id_p=\id_{Gp}$.
\end{defn}
...
\begin{defn}
If $(\mathcal{C},\cdot)$ (where $\mathcal{C}$ is a semicategory) is an actionable semicategory, then we can define the operation $\supfun{}$ by $\supfun{f}x = f\cdot x$.
\end{defn}

\begin{obvious}
The above defines a semicategory action $(\mathcal{C},\supfun{})$.
\end{obvious}
...
\begin{obvious}
If it is an ordered actionable semicategory, then the result is a ordered semicategory action.
\end{obvious}

\begin{obvious}
If it is an actionable semicategory with identities, then the result is an ordered semicategory action with identities.
\end{obvious}

\section{From semicategory action with identities to actionable semicategory}

Let $(\mathcal{C},\supfun{},p\mapsto\id_p)$ (where $\mathcal{C}$~is a semicategory) be an ordered semicategory action with identities, each object set having greatest element~$\top$.

\begin{prop}
$\supfun{f}x = \im(f\circ\id_x)$.\footnote{$\im f \eqdef \supfun{f}\top$.}
\end{prop}
...
By analogy with the above proposition, we can define $g\cdot f = \id_{\im(g\circ f)}$.
...
\begin{rem}
It is even a universal algebra variety~\cite{varieties-props} in the case of $\sqcap$-semilattice.
\end{rem}
...
\item $\id_{((\id_{f_1}\cdot f_0)\circ g_0)}\cdot((\id_{f_0}\cdot f_1)\circ(\id_{g_0}\cdot g_1)) = (\id_{f_0}\cdot f_1)\circ(\id_{g_0}\cdot g_1)$ (ditto)\footnote{$((f_0\sqcap f_1)\circ g_0)\sqcap((f_0\sqcap f_1)\circ(g_0\sqcap g_1)) = (f_0\sqcap f_1)\circ(g_0\sqcap g_1)$};
...
My research (mainly in~\cite{volume-1}) shows that all kinds of spaces met in the  traditional general topology:
\begin{itemize}
...
\item Cauchy spaces
\end{itemize}
are embedded into ordered semicategory actions and, because for these kinds of spaces the ordered semicategory actions have identities, are embedded into the algebra variety of actionable ordered semigroups. That is all general topology is an algebra, moreover all of it is the same algebraic variety (yes, even metric spaces).

This makes ordered semicategory actions immensely important, about on-par with group theory.
...
There are no competing interests in regard to content of this paper.
