...
\item \fxnote{Can we prove $p\sqsubseteq q\Rightarrow\id_p\sqsubseteq\id_q$? If not, add to above axioms.}
\item \fxnote{Also add axioms $\id_p\sqsubseteq 1$ and/or $\id_{\top}=1$?}
\end{enumerate}
\end{rem}
...
\fxnote{Need also to describe categories of objects defined in this section. Also describe embeddings with (ordered)     semicategory actions.}

\begin{rem}
An actionable semigroup is a universal algebra.
\end{rem}
...
\section{Actionable semigroup actions as universal algebras}

\begin{rem}
It is even a universal algebra variety\footnote{See here for \href{https://chatgpt.com/s/t_691e8f88da64819184591f9e310e1728}{properties of categories isomorphic to universal algebras}.} in the case of up or down semilattice.
\end{rem}
...
For $\sqcup$-semilattice, we need more axioms because it requires both $\sqcup$ and $\sqcap$ (for axiom~\ref{ax:ids}).

\begin{obvious}
The universal algebra signature for actionable semigroup with identities that is a $\sqcap$-semilattice:

Operations $\circ$, $\cdot$, $p\mapsto\id_p$, $\sqcup$ with axioms:
\begin{enumerate}
\item $h\circ(g\circ f) = (h\circ g)\circ f$ (semigroup);
\item $h\cdot(g\cdot f) = (h\circ g)\cdot f$ (actionable);
\item $a\sqcup(b\sqcup c) = (a\sqcup b)\sqcup c$ ($\sqcap$-semilattice)\footnote{$a\sqcap(b\sqcap c) = (a\sqcap b)\sqcap c$};
\item $a\sqcup b = b\sqcup a$ (ditto)\footnote{$a\sqcap b = b\sqcap a$};
\item $a\sqcup a = a$ (ditto)\footnote{$a\sqcap a=a$};
\item $(g_0\circ(f_0\sqcup f_1))\sqcup((g_0\sqcup g_1)\circ(f_0\sqcup f_1)) = (g_0\sqcup g_1)\circ(f_0\sqcup f_1)$ (ordered semigroup)\footnote{$(g_0\circ(f_0\sqcap f_1))\sqcap((g_0\sqcap g_1)\circ(f_0\sqcap f_1)) = (g_0\sqcap g_1)\circ(f_0\sqcap f_1)$};
\item $((f_1\sqcup f_0)\circ g_0)\sqcup((f_0\sqcup f_1)\circ(g_0\sqcup g_1)) = (f_0\sqcup f_1)\circ(g_0\sqcup g_1)$ (ditto)\footnote{$((f_0\sqcap f_1)\circ g_0)\sqcap((f_0\sqcap f_1)\circ(g_0\sqcap g_1)) = (f_0\sqcap f_1)\circ(g_0\sqcap g_1)$};
\item $(g_0\cdot(f_0\sqcup f_1))\sqcup((g_0\sqcup g_1)\cdot(f_0\sqcup f_1)) = (g_0\sqcup g_1)\cdot(f_0\sqcup f_1)$ (ordered actionable semigroup)\footnote{$(g_0\cdot(f_0\sqcap f_1))\sqcap((g_0\sqcap g_1)\cdot(f_0\sqcap f_1)) = (g_0\sqcap g_1)\cdot(f_0\sqcap f_1)$};
\item $(((f_1\sqcup f_0)\cdot g_0))\sqcup((f_0\sqcup f_1)\cdot(g_0\sqcup g_1)) = (f_0\sqcup f_1)\cdot(g_0\sqcup g_1)$ (ditto)\footnote{$((f_0\sqcap f_1)\cdot g_0)\sqcap((f_0\sqcap f_1)\cdot(g_0\sqcap g_1)) = (f_0\sqcap f_1)\cdot(g_0\sqcap g_1)$}.
\item ??Not enough axioms linking~$\sqcap\leftrightarrow\id$.
\end{enumerate}
\end{obvious}
