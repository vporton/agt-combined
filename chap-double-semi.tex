...
\begin{thm}
$(\mathcal{C},\supfun{}'|_{\setcond{\id_x}{x\in A,A\in\Ob\mathcal{C}}})$ is an ordered semicategory action naturally isomorphic (by an isomorphism in $\mathbf{OrdSemiCatAct}$) to the original one $(\mathcal{C},\supfun{})$.
\end{thm}

\begin{proof}
Denote $\Phi$ the functor that maps an object $(A,\circ,\supfun{})$ to $(A,\circ,\supfun{}')$ and a morphism $(F,G)$ to $(F,p\mapsto\id_p)$ and $\Psi$ the unique?? functor that maps an object $(A,\circ,\supfun{}')$ to $(A,\circ,\supfun{})$ and a morphism $(F,G)$ to $(F,\id_p\mapsto p)$.
\end{proof}
