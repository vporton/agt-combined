...
That is it $\mathbf{OrdSemiCat}$ with full faithful functor $\mathbf{OrdSemiCat}\to\mathbf{Pos}$.
\end{defn}
...
where $f:\mathcal{C}\to\mathcal{D}$ and $b\circ f=a$ and there is a full faithful functor~$\supfun{}$ such that where $a,b\in\Mor\mathbf{Arr}(\mathbf{Pos})$ that is $a$ and~$b$ are monotone functions, that is:
