...
Fix an ordered semicategory action with identities $(\mathcal{C},\supfun{},p\mapsto\id_p)$, where $\mathcal{C}$ is an ordered semicategory.
...
~
\begin{enumerate}
\item $(\mathcal{C},\supfun{}')$ is an ordered semicategory action naturally isomorphic (by an isomorphism in $\mathbf{OrdSemiCatAct}$) to the original one $(\mathcal{C},\supfun{})$.
\item $(\mathcal{C},\cdot')$ is an ordered actionable semicategory, corresponding (as above) to the ordered semicategory action $(\mathcal{C},\supfun{}')$.
\end{enumerate}
\end{thm}
