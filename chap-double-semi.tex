...
\begin{defn}
The category~$\mathbf{SemiOrdSemiCatAct}$ of \emph{semi-ordered semicategory actions} is small ordered semicategories with semifunctors between them as morphisms and a full faithful functor \[ \supfun{}:\mathbf{SemiCat}\to\mathbf{Pos} \] with the identity $a\sqsubseteq b\Rightarrow\supfun{a}x\sqsubseteq\supfun{b}x$.

In other words, $\mathbf{SemiOrdSemiCatAct}$ is small semi-ordered semicategories with morphisms conforming to identities:
\begin{enumerate}
...
\begin{defn}
The category~$\mathbf{OrdSemiCatAct}$ of \emph{ordered semicategory actions} is
$\mathbf{OrdSemiCat}$ full faithful functor
\[ \supfun{}:\mathbf{OrdSemiCat}\to\mathbf{Pos} \]
with with the identity $a\sqsubseteq b\Rightarrow\supfun{a}x\sqsubseteq \supfun{b}x$.

In other words, $\mathbf{OrdSemiCatAct}$ is small semi-ordered semicategories with morphisms conforming to identities:
\begin{enumerate}
\item $\supfun{b\circ a}=\supfun{b}\circ\supfun{a}$;
\item $a\sqsubseteq b\Rightarrow\supfun{a}x\sqsubseteq \supfun{b}x$;
\item $x\sqsubseteq y\Rightarrow\supfun{a}x\sqsubseteq\supfun{a}y$.
\end{enumerate}
\end{defn}

\begin{obvious}
\[ \mathbf{OrdSemiCatAct}\text{ is a wide subcategory of }\mathbf{SemiOrdSemiCatAct}. \]
\end{obvious}
