...
\begin{defn}
The category~$\mathbf{OrdSemiCat}$ of ordered semicategories is small ordered semicategories with monotone semifunctors as morphisms.
\end{defn}

\begin{defn}
The category~$\mathbf{SemiOrdSemiCatAct}$ of semi-ordered semicategory actions is small semi-ordered semicategories with monotone semifunctors between them as morphisms.
\end{defn}

\begin{defn}
The category~$\mathbf{OrdSemiCatAct}$ of ordered semicategory actions is the wide subcategory of~$\mathbf{SemiOrdSemiCatAct}$
consisting of semifunctors~$(a,b,f)$ with the additional requirement $a\sqsubseteq b\Rightarrow\supfun{a}x\sqsubseteq \supfun{b}x$ for all $a,b\in\Hom(x,y)$, $x,y\in\Ob\mathcal{C}$.

Alternatively, it is $\mathbf{SemiCat}$ with full faithful functor
\[ \supfun{}:\mathbf{SemiCat}\to\mathbf{Pos} \]
for which holds $\supfun{b}\circ a=\supfun{b\circ a}$.
\end{defn}

\begin{prop}
$\mathbf{OrdSemiCatAct}$ can be equivalently (with an obvious isomorphism) be defined as
$\mathbf{OrdSemiCat}$ with full faithful functor
\[ \supfun{}:\mathbf{OrdSemiCat}\to\mathbf{Pos} \]
for which holds $\supfun{b}\circ a=\supfun{b\circ a}$.
\end{prop}

\begin{proof}
Note one-to-one correspondance between monotone morphisms and morphisms of~$\mathbf{Pos}$.
\end{proof}
