...
\begin{prop}
The above with the same semigroup operation~$\circ$ and the same identities defines an actionable semicategory with identities.
\fxerror{Identities can't be the same, because the set of objects is not the same.}
\end{prop}
...
Identities axioms match, because the~$\circ$ operation is the same.
\end{proof}
