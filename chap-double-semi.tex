...
\begin{defn}
The category~$\mathbf{SemiOrdSemiCat}$ of semi-ordered semicategories is ??.
\end{defn}
...
\begin{defn}\fxerror{Seems wrong.}
The category~$\mathbf{OrdSemiCat}$ of ordered semicategories is the wide subcategory of~$\mathbf{SemiOrdSemiCat}$
consisting of semifunctors~$a$ such that $a\sqsubseteq b\Rightarrow\supfun{a}x\sqsubseteq \supfun{b}x$ for all $a,b\in\Hom(x,y)$, $x,y\in\Ob\mathcal{C}$.
\end{defn}
