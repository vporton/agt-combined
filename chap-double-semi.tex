...
[??Check that I have correct ``items'' in the tuples.]  

Fix an ordered semicategory action with identities $(A,\circ,\supfun{},p\mapsto\id_p)$.
...
~
\begin{enumerate}
\item $(A,\circ,\supfun{}')$ is an ordered semicategory action naturally isomorphic (by an isomorphism in $\mathbf{OrdSemiCatAct}$) to the original one $(A,\circ,\supfun{})$.
\item $(A,\circ,\cdot')$ is an ordered actionable semicategory, corresponding (as above) to the ordered semicategory action $(A,\circ,\supfun{}')$.
\end{enumerate}
\end{prop}
