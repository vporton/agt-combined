...
\emph{Semicategory action with identities} is an ordered semicategory~$S$ action~$\supfun{}$ together with a function $p\mapsto\id_p\in S$ \fxerror{Define the domain of $p\mapsto\id_p$} such that whenever these equalities are defined
\begin{enumerate}
...
Let write the full universal algebra signature for actionable semigroup with identities ?? up semilattice:
